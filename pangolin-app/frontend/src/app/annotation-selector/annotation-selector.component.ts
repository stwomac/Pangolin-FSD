...
export class AnnotationSelectorComponent {
  constructor(private annotationServices : AnnotationServices,private reportServices : ReportServices) {}
  ...
  shouldBeChecked(annotation : Annotation) : boolean {
    ...
    return this.report.annotations.some(ele => ele.annotation === annotation.annotation);
  }
  ...
  updateReport() {
    if (this.report)
      this.reportServices.update(this.report);
  }
  ...
  checkBoxInputChanged(event : any) {
    ...
    if (!event.event.target.checked)
      this.report.annotations = this.report.annotations.filter(ele => ele.annotation !== event.annotation.annotation);
    else if (!this.report.annotations.some(ele => ele.annotation === event.annotation.annotation))
